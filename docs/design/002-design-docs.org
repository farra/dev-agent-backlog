...
Documents use sequential numbering (001, 002, 003...). Classification is
handled via =#+CATEGORY:= in the document frontmatter.

See =docs/design/README.org= for valid categories.
...
- [2026-01-10] Replaced category-based number ranges with sequential numbering.
  Documents now use =#+CATEGORY:= frontmatter for classification. See design
  doc 010 for details.
