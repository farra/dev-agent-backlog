...
Bidirectional sync is handled by one skill (=claude-tasks-sync=):

#+begin_quote
"When using Claude Tasks, sync them to backlog.org"
#+end_quote

The skill does:
1. When Claude creates/updates Tasks, ensure matching backlog entry exists
2. If backlog entry exists, verify link is present
3. If backlog entry doesn't exist, create it with =:CLAUDE_TASK:= link

Going the *other* direction (backlog → Claude Task) only happens at execution
time—when we queue a single task OR an entire design doc for active work.

#+begin_example
Claude Tasks → backlog.org: Always (via sync skill)
backlog.org → Claude Tasks: Only at execution time
#+end_example

This keeps backlog.org as the authoritative human-readable view while letting
Claude use its native primitives for coordination.
...
New skill that monitors Claude Task creation/updates and ensures matching
backlog entries exist:
- When Claude creates a Task, check if backlog entry exists
- If exists, verify link is present
- If not, create backlog entry with =:CLAUDE_TASK:= link
- Single direction: Claude Tasks → backlog.org (always)

This is the core sync mechanism. The reverse direction (backlog → Claude Task)
happens only at execution time via task-start or queue-design-doc.
